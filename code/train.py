...
vocab, train_x, test_x, ref_x, overall_maxlen = dataset.get_data_totrain(args.domain, vocab_size=args.vocab_size, maxlen=args.maxlen)
...
ref_gen = sentence_batch_generator(ref_x, args.batch_size)
#print(ref_gen.next())
...
for ii in xrange(args.epochs):
    ...
    for b in tqdm(xrange(batches_per_epoch)):
        ...
        ref_input = ref_gen.next()
        #print(sen_input[0].shape)
        #print(neg_input[0].shape)
        #print(ref_input[0].shape)

        #batch_loss, batch_max_margin_loss = model.train_on_batch([sen_input, neg_input, ref_input], np.ones((args.batch_size, 1)))
        #get layer
    ...
    if loss < min_loss:
        min_loss = loss
        word_emb = K.get_value(model.get_layer('word_emb').W)
