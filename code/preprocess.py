...
def preprocess_train(domain):
    ...
    f1 = codecs.open('../datasets/'+domain+'/refwords_train.txt', 'r', 'utf-8')
    out1 = codecs.open('../preprocessed_data/'+domain+'/refwords_train.txt', 'w', 'utf-8')

    for line, refword in zip(f, f1):
        tokens = parseSentence(line)
        #tokens_ref = parseSentence(refword)
        if len(tokens) > 0:
            out.write(' '.join(tokens)+'\n')
            #out1.write(' '.join(tokens_ref)+'\n')
            out1.write(refword.strip()+'\n')
def preprocess_ref(domain,filename):
    f = codecs.open('../datasets/'+domain+'/'+filename, 'r', 'utf-8')
    out = codecs.open('../preprocessed_data/'+domain+'/'+filename, 'w', 'utf-8')
...
def preprocess_test(domain):
    ...
    f3 = codecs.open('../datasets/'+domain+'/refwords_test.txt', 'r', 'utf-8')
    out3 = codecs.open('../preprocessed_data/'+domain+'/refwords_test.txt', 'w', 'utf-8')
    
    	
    for text, label, refword in zip(f1, f2, f3):
        label = label.strip()
...
def preprocess(domain):
    ...
    #print '\t'+domain+' ref words train ...'
    #preprocess_ref(domain,'refwords_train.txt')
    #print '\t'+domain+' ref words test ...'
    #preprocess_ref(domain,'refwords_test.txt')
'''
