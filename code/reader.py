...
def read_dataset(domain, phase, vocab, maxlen):
    assert domain in {'restaurant'}
    assert phase in {'train', 'test', 'refwords_train', 'refwords_test'}
...
def get_data_totrain(domain, vocab_size=0, maxlen=0):
    ...
    print('  referencial words')
    ref_x, ref_maxlen = read_dataset(domain, 'refwords_train', vocab, maxlen)
    maxlen = max(train_maxlen, test_maxlen, ref_maxlen)
    return vocab, train_x, test_x, ref_x, maxlen
    
def get_data_totest(domain, vocab_size=0, maxlen=0):
    print('Reading data from', domain)
    print(' Creating vocab ...')
    vocab = create_vocab(domain, maxlen, vocab_size)
    print(' Reading dataset ...')
    ...
    test_x, test_maxlen = read_dataset(domain, 'test', vocab, maxlen)
    print('  referencial words')
    ref_x, ref_maxlen = read_dataset(domain, 'refwords_test', vocab, maxlen)
    maxlen = max(train_maxlen, test_maxlen, ref_maxlen)
    return vocab, train_x, test_x, ref_x, maxlen


if __name__ == "__main__":
    vocab, train_x, test_x, maxlen = get_data_totrain('restaurant')
    print(len(train_x))
    print(len(test_x))
    print(maxlen)
