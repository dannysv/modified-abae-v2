...
class Average(Layer):
    ...
    def call(self, x, mask=None):
        ...
        if mask is not None:
            ...
            x = x * mask
        #print("klfdjasaslkdjflasd\n\n\n")
        #print(K.eval(K.sum(x, axis=-2) / K.sum(mask, axis=-2)))
        return K.sum(x, axis=-2) / K.sum(mask, axis=-2)

    def get_output_shape_for(self, input_shape):
        #print("necesite de este")
        return input_shape[0:-2]+input_shape[-1:]
    
    def compute_mask(self, x, mask=None):
        #print('de este tambien')
        return None

class W_reference(Layer):
    def __init__(self, **kwargs):
        print('obvio')
        self.supports_masking = True
        super(W_reference, self).__init__(**kwargs)
